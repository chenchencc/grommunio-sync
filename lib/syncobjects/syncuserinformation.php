...
class SyncUserInformation extends SyncObject {
	...
	public function __construct() {
		$mapping = [SYNC_SETTINGS_PROP_STATUS => [self::STREAMER_VAR => 'Status',
		...
		if (Request::GetProtocolVersion() >= 12.0) {
			$mapping[SYNC_SETTINGS_EMAILADDRESSES] = [self::STREAMER_VAR => 'emailaddresses',
				self::STREAMER_ARRAY => SYNC_SETTINGS_SMPTADDRESS, ];
		}

		if (Request::GetProtocolVersion() >= 14.1) {
			unset($mapping[SYNC_SETTINGS_EMAILADDRESSES]);
			$mapping[SYNC_SETTINGS_ACCOUNTS] = [self::STREAMER_VAR => 'accounts',
